...
def finance_quater_consen(code, name, gubun='D'):
    url = f'https://comp.fnguide.com/SVO2/json/data/01_06/01_A{code}_Q_{gubun}.json'
    res = requests.get(url)
    temp = json.loads(res.content)
    df = pd.DataFrame(temp['comp'])
...
def save(df, filename=f'종목별_컨센서스_250630.csv'):  # 엑셀 저장 함수
    try:
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(filtered_co))):  # 우선 10개만 조회
        try:
            cd = temp['Co'][i]['cd'][1:]
            nm = temp['Co'][i]['nm']
            # quater_df = finance_quater_consen(cd, nm)
